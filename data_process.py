...
def data_process1():  # 分类数据预处理
    ...
    data = pd.DataFrame(data, columns=['留言主题', '一级标签'])
    ...
    data_cut = data['message'].drop_duplicates()
    data_cut = data_cut.astype(str)
    data_cut = data_cut.apply(lambda x: re.sub('[A-Z]', '', x))  # 去除字母和数字
    data_cut = data_cut.apply(lambda x: re.sub('[0-9]', '', x))
...
def data_process2():  # 聚类数据预处理
    file2 = 'D:/programs/zhihuizhengwu/newdict.txt'
    file4 = 'D:/programs/zhihuizhengwu/附件2（测试数据）.xlsx'
